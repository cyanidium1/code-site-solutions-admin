...
const bodyUk = [
  ...
  li("**Месенджери й сервіси** — Telegram, Slack і RSS-читалки тягнуть фавікон для превʼю посилання."),
  p("Видача — найнедооціненіший пункт списку: ви доводите сторінку до топ-5, а поруч із заголовком стоїть сірий глобус, тоді як у конкурента — впізнаваний знак."),
  ...
  p("Найпоширеніша помилка — механічно зменшити логотип до 32 пікселів: лок-ап із назвою, слоганом і тонкими лініями перетворюється на сіру пляму. [Тренди веб-дизайну](/blog/trendy-veb-dyzainu-2026) змінюються щороку, а вимога впізнаваності на 16 пікселях не змінюється з 1999-го."),
  ...
  p("Браузер малює вкладки на світлому або темному тлі залежно від теми системи. Чорна монограма з прозорим фоном ідеально виглядає у світлій темі й повністю зникає в темній. Рішень два: непрозора плашка фірмового кольору або SVG-фавікон із медіазапитом prefers-color-scheme усередині файлу — тоді іконка перемикає колір сама. Логіка та сама, що й у виборі [теми для всього сайту](/blog/temna-chy-svitla-tema-saitu)."),
  ...
  p("Окремої ціни на фавікон у нас немає: він входить у будь-який пакет — і в [лендінг](/landing), і в корпоративний сайт, і в магазин. Дизайнер готує набір іконок разом із макетом, розробник підключає файли й маніфест. Це частина нормальної здачі проєкту — так само, як [адаптивна верстка](/blog/shcho-take-adaptyvna-verstka) чи сторінка [цін](/pricing) без сюрпризів."),
...
const bodyRu = [
  ...
  li("**Мессенджеры и сервисы** — Telegram, Slack и RSS-читалки тянут фавикон для превью ссылки."),
  p("Выдача — самый недооценённый пункт списка: вы выводите страницу в топ-5, а рядом с заголовком стоит серый глобус, тогда как у конкурента — узнаваемый знак."),
  ...
  p("Самая частая ошибка — механически уменьшить логотип до 32 пикселей: лок-ап с названием, слоганом и тонкими линиями превращается в серое пятно. [Тренды веб-дизайна](/ru/blog/trendy-veb-dizayna-2026) меняются каждый год, а требование узнаваемости в 16 пикселях не менялось с 1999-го."),
  ...
  p("Браузер рисует вкладки на светлом или тёмном фоне в зависимости от темы системы. Чёрная монограмма с прозрачным фоном отлично выглядит в светлой теме и полностью исчезает в тёмной. Решений два: непрозрачная плашка фирменного цвета или SVG-фавикон с медиазапросом prefers-color-scheme внутри файла — тогда иконка переключает цвет сама. Логика та же, что и при выборе [темы для всего сайта](/ru/blog/tyomnaya-ili-svetlaya-tema-sayta)."),
  ...
  p("Отдельной цены на фавикон у нас нет: он входит в любой пакет — и в [лендинг](/ru/landing), и в корпоративный сайт, и в магазин. Дизайнер готовит набор иконок вместе с макетом, разработчик подключает файлы и манифест. Это часть нормальной сдачи проекта — ровно как адаптивная вёрстка или страница [цен](/ru/pricing) без сюрпризов."),
...
const bodyEn = [
  tldr("Favicon in short", [
    ...
    "The set: favicon.ico with 16 and 32 px, a 180×180 apple-touch-icon, 192 and 512 px PNGs in the manifest.",
    ...
    "Common mistakes: 16×16 only, a transparent background on dark theme, no apple-touch-icon.",
    "Every package we build includes the icon set — it is not billed separately.",
  ]),

  p("A favicon is the small square site icon a browser shows in the tab, in bookmarks, in history, and next to your title in Google mobile results. Technically it is an image between 16×16 and 512×512 pixels in ICO, PNG or SVG format, placed in the site root and referenced from the head with a link rel=icon tag."),
  p("The name comes from favorite icon: in 1999 Internet Explorer 5 started showing site icons in the Favourites list. It is now the only branding that survives when a tab is squeezed to twenty pixels."),

  h2("In plain terms"),
  p("Picture a shelf of twenty identical white boxes. The favicon is the sticker that lets someone find their box without reading a label. With twenty tabs open the title is cut to a couple of characters, and the icon is all that is left — so it has to read at thumbnail size, not look beautiful at full size."),
  ...
  li("**Bookmarks and history** — in a long list the eye finds an icon faster than it reads a title."),
  li("**Google mobile results** — the favicon sits left of the result title on phones. A snippet with a crisp icon reads as more solid than one with an empty circle."),
  ...
  p("Search results are the most underrated item on that list: you push a page into the top five, then sit beside a competitor whose branded mark is showing while yours is a grey globe."),
  ...
  table(
    ...
    [
      ...
      ["32×32", "ICO or PNG", "Retina tabs, Windows taskbar", "Yes"],
      ["48×48 and multiples", "PNG/ICO in root", "Google mobile results", "Yes"],
      ["180×180", "PNG, apple-touch-icon", "iPhone/iPad home screen", "Yes"],
      ["192×192", "PNG in manifest", "Android home screen, PWA", "Yes"],
      ["512×512", "PNG in manifest", "PWA splash screen, install prompt", "Yes"],
      ["Vector", "SVG", "Modern browsers, dark-theme switching", "Recommended"],
    ],
  ),
  p("The minimum kit: one favicon.ico holding both 16 and 32 px, apple-touch-icon.png at 180 px, icon-192.png and icon-512.png in the manifest. SVG is a bonus, not a replacement."),

  h2("Making one from your logo"),
  p("The most common mistake is to scale the finished logo down to 32 pixels: a lockup with a company name, a strapline and hairline strokes becomes a grey smudge. [Web design trends](/en/blog/web-design-trends-2026-to-ignore) change every year, but legibility at 16 pixels has been the rule since 1999."),
  ...
  p("Browsers draw tabs on a light or dark background depending on the system theme. A black monogram on a transparent background looks perfect in light mode and vanishes in dark mode. Two fixes work: an opaque tile in your brand colour, or an SVG favicon with a prefers-color-scheme media query inside the file, so the icon switches colour itself. Same reasoning as the wider [dark vs light theme](/en/blog/dark-vs-light-website-theme) decision."),

  h2("How to wire it up"),
  p("The files sit in the root and are referenced from the head:"),
  ...
  p("Keep favicon.ico in the root itself: browsers and Google's crawler request /favicon.ico directly even when no tag is present. After swapping an icon, check in a private window — favicons are cached aggressively."),

  h2("Common mistakes"),
  li("**16×16 only.** The icon blurs on high-density screens and fails Google's multiple-of-48 requirement."),
  li("**A transparent background under a dark mark.** In a dark browser theme the icon disappears entirely."),
  li("**No apple-touch-icon.** Added to an iPhone home screen, the site gets a screenshot instead of an icon."),
  li("**The whole logo scaled down.** Type and fine detail merge into a blob."),
  li("**An icon the crawler cannot reach.** Blocked in robots.txt or erroring means Google shows nothing."),
  li("**A redesign that skipped the icon.** New site, five-year-old favicon still in the root."),

  h2("What it costs"),
  p("We do not price favicons separately — the icon set is part of every build, whether that is a [landing page](/en/landing), a corporate site or an online store. The designer prepares the icons alongside the layout, the developer wires up the files and the manifest. It is part of shipping properly, not a line item; see what each package covers on our [pricing page](/en/pricing)."),
  p("If your site is live and only the icon is missing, it is half an hour under a [support](/en/support) plan."),
